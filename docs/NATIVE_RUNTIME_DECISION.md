...
The later owner-approved exception to the original no-helper wording covers only machine-inactivity protection: a bounded elevated live broker and a one-shot elevated restore helper. The broker starts only after explicit UAC for that operation, restores the exact original machine-inactivity value if the main process disappears, then exits. If the broker does not restore, the same account can approve the one-shot administrator helper on next launch to restore its recorded original. Neither is a general hidden helper, service, task or RunOnce recovery path. Other pending policy recovery still relies on ARR where invoked and the durable next-launch journal. The old PowerShell Dispatch 004 remains prohibited. Issue #3 records the former PowerShell security incident; the unsigned native build has no universal EDR certification.
